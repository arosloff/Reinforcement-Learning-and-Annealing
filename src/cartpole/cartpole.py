...
def choose_action(state, mdp_data):
    ...
    # *** START CODE HERE ***
    p = mdp_data.get('transition_probs')
    v = mdp_data.get('value')

    zero = 0
    one = 0

    for i in range(v.shape[0]):
        zero += (p[state][i][0] * v[i])
        one  += (p[state][i][1] * v[i])
...
def update_mdp_transition_counts_reward_counts(mdp_data, state, action, new_state, reward):
    ...
    mdp_data.get("transition_counts")[state, new_state, action] += 1
    
    if (reward == -1):
        mdp_data.get("reward_counts")[new_state, 0] += 1
    mdp_data.get("reward_counts")[new_state, 1] += 1
...
def update_mdp_transition_probs_reward(mdp_data):
    ...
    # *** START CODE HERE ***
    reward = mdp_data.get("reward")
    p      = mdp_data.get("transition_probs")
    tc     = mdp_data.get("transition_counts")

    tempTc = tc.copy()

    # zeroSumVector = np.zeros(reward.shape[0])
    # oneSumVector  = np.zeros(reward.shape[0])

    for i in range(reward.shape[0]):
        zeroSumVector = np.sum(tempTc[i, :, 0])
        oneSumVector  = np.sum(tempTc[i, :, 1])
        if zeroSumVector != 0:
            p[i, :, 0] = tempTc[i, :, 0] / zeroSumVector
        if oneSumVector != 0:
            p[i, :, 1] = tempTc[i, :, 1] / oneSumVector

    mdp_data["transition_probs"] = p

    for i in range(reward.shape[0]):
        if mdp_data.get("reward_counts")[i, 1] != 0:
            mdp_data.get("reward")[i] = -(mdp_data.get("reward_counts")[i, 0] / mdp_data.get("reward_counts")[i, 1])
...
def update_mdp_value(mdp_data, tolerance, gamma):
    ...
    while True:
        ...
        zero  = np.zeros(v.shape[0])
        one   = np.zeros(v.shape[0])
        for k in range(v.shape[0]):
            zero[k] = (np.dot(p[k, :, 0], v))
            one [k] = (np.dot(p[k, :, 1], v))
        for k in range(v.shape[0]):
            if (gamma * zero[k]) > (gamma * one[k]):
                new_v[k] = (gamma * zero[k]) + mdp_data.get('reward')[k]
            else:
                new_v[k] = (gamma * one[k])  + mdp_data.get('reward')[k]

        if (it > 0):
            if (np.max(np.abs(new_v - v)) < tolerance):
                mdp_data['value'] = new_v
                return it == 1
        v = new_v
        it += 1
    # *** END CODE HERE ***
